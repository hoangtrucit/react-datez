...
class ReactDatez extends Component {
  ...
  initialiseMonthCalendar() {
    ...
    for (let i = 1; i < 13; i += 1) {
      months.push(
        <a
          href=""
          className="rdatez-month"
          key={`${this.state.currentMonthYear}-months-${i}`}
          onClick={e => this.clickMonth(e, i)}
        >
          {moment(i, "M").format("MMM")}
        </a>
      );
    }
  ...
  initialiseYearCalendar() {
    ...
    for (
      let i = this.state.visibleYear.format("YYYY");
      i > this.state.visibleYear.format("YYYY") - 12;
      i -= 1
    ) {
      years.push(
        <a
  ...
  initialiseCalendar() {
    ...
    return calendar.map(i => (
      <div key={`calendar-${i}`}>
        <header className="rdatez-calendar-title" key={`month-header-${i}`}>
          {moment(this.state.currentMonthYear, "M YYYY")
            .add(i, "months")
            .format("MMMM YYYY")}
        </header>
  ...
  selectedDate(date) {
    if (
      moment(this.state.selectedDate, this.props.dateFormat).diff(
        moment(date, this.props.dateFormat),
        "days"
      ) === 0
    ) {
      return true;
  ...
  isPast(date) {
    if (
      moment()
        .startOf("day")
        .diff(moment(date, this.props.dateFormat), "days") > 0
    ) {
      return true;
    }
  ...
  disabledTodayJump() {
    ...
    if (
      (this.props.startDate && this.isBeforeStartDate(moment())) ||
      (this.props.endDate && this.isAfterEndDate(moment()))
    ) {
      this.setState({
        disabledToday: true
      });
  ...
  jumpToToday(e) {
    ...
    const date = !this.props.allowFuture
      ? moment().format(this.props.dateFormat)
      : moment().format(this.props.dateFormat);

    // catch the date range
    if (
      (this.props.startDate && this.isBeforeStartDate(date)) ||
      (this.props.endDate && this.isAfterEndDate(date))
    ) {
      return false;
    }
  ...
  nextButton() {
    if (this.state.yearJumpOpen) {
      ...
      this.setState({
        currentMonthYear: moment(this.state.currentMonthYear, "M YYYY")
          .add(1, "months")
          .format("M YYYY"),
  ...
  renderCalendar(offset) {
    const date = offset
      ? moment(this.state.currentMonthYear, "M YYYY").add(offset, "months")
      : moment(this.state.currentMonthYear, "M YYYY");
    ...
    for (let i = 1; i <= daysInMonth; i += 1) {
      ...
      if (i === 1) {
        calendar.push(
          <div
            className={`rdatez-day-spacer weekday-${day.day()}`}
            key={`${day.format("MY")}-spacer`}
          />
        );
      }

      const dayClasses = classnames(
        `rdatez-day weekday-${day.day()} ${day.format("M-YYYY")}-${i}`,
        {
      ...
      calendar.push(
        <a
          ...
          key={`${day.format("DMY")}-${i}`}
          onClick={e => this.clickDay(e, dayDate)}
          tabIndex="-1"
        >
          {i}
        </a>
          ...
          >
            <div>
              <header className="rdatez-header">
                ...
                <button
                  type="button"
                  className="rdatez-btn rdatez-btn-prev"
                ...
                <button
                  type="button"
                  className="rdatez-btn rdatez-btn-next"
                ...
                {this.props.yearJump && (
                  <button
                    type="button"
                    className="rdatez-btn rdatez-btn-year"
                    onClick={this.toggleYearJump}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25">
                ...
                {!this.state.disabledToday && (
                  <button
                    type="button"
                    className="rdatez-btn rdatez-btn-today"
                    onClick={e => this.jumpToToday(e)}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25">
